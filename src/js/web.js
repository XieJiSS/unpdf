...
String.prototype.format = function format () { //python str的format方法的阉割实现
    function toStr (o) {
        if(o === undefined) {
            return "";
        } else if (o === null) {
    ...
    for(var i = 0; i < arguments.length; i++) {
        if(str.includes(`{${ i }}`)) {
            str = str.replace(new RegExp(`\\{${ i }\\}`, "g"), toStr(arguments[i])); // 危险操作
        } else {
...
function search (str, language, ftype) {
    ...
    let jqXHR = $.get(ctx_search.format(encode(str), encode(lang)), data => {
        ...
        for(var i = 0; i < nodes.length; i++) {
            ...
            }).replace(/联\s?合\s?国/, "联合国").slice(1).split("\n");
            let path = lines[0];
            lines.shift(); // PATH
            lines.shift(); // TITLE
            let text = lines.join("\n");
...
function download(p, l, t, ftype="PDF", isTitle=false) {
    ...
    $.get(u, data => {
        ...
        let w = window.open("https://daccess-ods.un.org" + redir, `Download PDF: ${ p } ${ t || "TITLE NOT AVAILABLE"}`);
        $("#path").val(p);
        if (isTitle) {
...
$("input#path")[0].addEventListener("keydown", ev => {
