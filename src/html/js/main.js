...
function PointCollection() {
    this.mousePos = new Vector(-150, -150);
...
function Point(x, y, z, size, color) {
    ...
    this.draw = function (bubbleShape, dx, dy) {
        ...
        if (bubbleShape == "square") {
            ...
            ctx.beginPath();
            ctx.arc(this.curPos.x + dx + window.leftOffset, this.curPos.y + dy + window.topOffset, this.radius, 0, Math.PI * 2, true);
            ctx.fill();
        }
    };
}
...
function updateCanvasDimensions() {
    ...
    $("#myCanvas")[0].getContext("2d").scale(window.scaleValue, window.scaleValue);//TODO
...
function onMove(e) {
    if (pointCollection) {
        pointCollection.mousePos.set((e.pageX - canvas.offset().left) / window.scaleValue - window.leftOffset, (e.pageY - canvas.offset().top) / window.scaleValue - window.topOffset);
    }
}
...
function draw(reset) {
    ...
    ctx.clearRect(0, 0, canvasWidth / window.scaleValue, canvasHeight / window.scaleValue);
...
$(window).mouseleave(function () {
    window.reset = false;
    if (pointCollection) {
        pointCollection.mousePos.set(-150, -150);
    }
});
...
window.scaleValue = 0.4;
window.leftOffset = 150;
window.topOffset = 12;
...
var grass = [79, 80, 70];
var testColor = [205, 100, 60];
var letterColors = [red, orange, green, blue, purple, testColor, [155, 100, 39]];
